...
import {QuestionValue, Question, Answer} from '../../_models/question';
...
export class AddQuestionModalComponent implements  OnInit{
    ...
    save(title: string, values: string) {
    ...
    selectType() {
        ...
        if (this.question.type.toString() === 'boolean') {
    ...
    private isConditionalQuestionType() {
        return this.question.type.toString() === 'BOOLEAN';
    }
    ...
    public isChoiseQuestionType() {
        return ['BOOLEAN', 'MULTI_CHOICE_MULTI', 'MULTI_CHOICE_SINGLE'].includes(this.question.type.toString());
    }

    public isTextQuestionType() {
        return ['DATE_TYPE', 'SHORT_ANSWER', 'LONG_ANSWER', 'NUMERICAL'].includes(this.question.type.toString());
    }
